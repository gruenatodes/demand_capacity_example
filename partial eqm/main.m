...
for iA = 1:pp.NA
    ...
    pol_capacU(iA) = res.prod.avg_prod / res.max_ys * 100;
...
lh_capacU = @(t, states) normpdf( ... 
    data(t,:,1), pol_capacU(states), p.sigma_capacU);
lh_wwK = @(t, states) normpdf( ... 
    data(t,:,1), pol_wwK(states), p.sigma_wwK);
