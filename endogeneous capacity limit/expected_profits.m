...
if ~full_capacity
    b_bar_1 = pr ^ (2 - p.alpha + p.alpha * p.sigma) * ...
    ...
    exp_profits = (agg.P / pr) ^ (p.sigma - 1) * agg.Y * pe_bbar1 - ...
        agg.w ^ ( (2 - 2 * p.alpha) / 2 - p.alpha) * ...
            agg.P ^ (p.sigma - 1) * agg.Y / (z * pr ^ p.sigma) ^ (2 / (2 - p.alpha)) * ...
    ...
    exp_profits = (agg.P / pr) ^ (p.sigma - 1) * agg.Y * pe_bhat - ...
        ...
        (1 - F_bbar2) * (pr*z) ^ (1 / p.alpha) / agg.w ^ ((1 - p.alpha) / p.alpha) * ...
            k * (1 - p.alpha) ^ ( (1 - p.alpha) / p.alpha) * p.alpha - ...
        (1 - F_bhat) * p.chi;
